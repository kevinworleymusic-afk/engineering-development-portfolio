# Master Engineering Development Calendar

This calendar summarizes development across the full technical-audio portfolio. It is intentionally brief: each entry links to the repository or detailed record where the actual work, methods, results, and limitations are documented.

[July 2026](#july-2026) · [August 2026](#august-2026) · [Development categories](#development-categories)

## Development Categories

The same marker identifies each category in the legend, calendar cells, gateways, and detailed calendars:

- [🟦 **Critical Listening**](Areas/critical-listening.md)
- [🟧 **Automotive Audio & Technical Skills**](Areas/automotive-audio.md)
- [🟪 **DSP, Software & Electronics**](Areas/dsp-and-software.md)
- [🟩 **Portfolio & Planning**](Areas/portfolio-and-planning.md)
- [🟨 **Professional Networking & Acoustical Consulting**](Areas/acoustical-consulting.md)

**Status:** ✓ **Recorded** · ◇ **Planned**

Recorded entries use short action phrases so the calendar shows what was actually done; planned entries state the intended next action.

Repository and document additions use the date of the first relevant GitHub commit. Sessions, calls, and outreach use their recorded real-world dates. Entries describing multi-day strategy or procedure development are period-level records and do not imply that a particular file was created that day.

## Weekly-to-Daily Flow

At the beginning of each week, choose the processes or objectives for the relevant categories. Each scheduled item will appear in its day cell using the category marker, linked label, status, and a very short process description:

[🟦 **Critical Listening**](Areas/critical-listening.md)<br>◇ EQ + compression warmup

Only scheduled categories appear on a given day. Multiple categories are separated into their own marked lines. The category opens its pathway, while completed work can link onward to the detailed calendar, daily log, or project evidence. Unplanned future dates remain date-only.

Use the [populated pathway plans](Plans/) to review the objectives behind recorded calendar work, and the shared [monthly and weekly templates](Templates/) to plan future cycles. Non-listening evidence remains in its project, document, calendar, or contact record.

<details>
<summary><strong>July 2026 — Portfolio and Development-System Foundation</strong></summary>

## July 2026

> **Reading guide:** This main view records concise outcomes in a phone-friendly weekly agenda. Select a date or category for the fuller record.

<details>
<summary><strong>Choose a July view</strong></summary>

- **Full month:** Continue below
- [July 1–12 — Pre-calendar period](Weeks/2026-07-01-12.md)
- [July 13–19 — Outreach and software foundation](Weeks/2026-07-13-19.md)
- [July 20–26 — Portfolio, guidance, and listening launch](Weeks/2026-07-20-26.md)
- [July 27–31 — Baselines, procedures, and planning](Weeks/2026-07-27-31.md)

</details>

### July 1–12 — Pre-calendar development period

The consolidated development calendar had not yet begun. No day-by-day entries are available for this period; this should not be interpreted as a period without work or progress.

### July 13–19 — Outreach and software foundation

| Date | Recorded outcomes |
|---|---|
| **Mon., July 13** | — No consolidated entry recorded. |
| [**Tue., July 14**](Areas/acoustical-consulting.md) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Contacted an acoustical consultant. |
| [**Wed., July 15**](Areas/acoustical-consulting.md) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Professional outreach and research period. |
| [**Thu., July 16**](Areas/acoustical-consulting.md) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Professional outreach and research period. |
| [**Fri., July 17**](Areas/acoustical-consulting.md) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Professional outreach and research period. |
| [**Sat., July 18**](Areas/dsp-and-software.md) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Initialized the Python audio analyzer.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Continued outreach and research. |
| [**Sun., July 19**](Areas/dsp-and-software.md) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Added Python analyzer features and documentation.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Continued outreach and research. |

### July 20–26 — Portfolio structure, expert guidance, and listening launch

| Date | Recorded outcomes |
|---|---|
| [**Mon., July 20**](Areas/acoustical-consulting.md) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Scheduled the Coffman Engineers call. |
| [**Tue., July 21**](Areas/portfolio-and-planning.md) | [🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Built the central engineering portfolio.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Added tests and MATLAB projects.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Prepared for the Coffman call. |
| [**Wed., July 22**](Areas/portfolio-and-planning.md) | [🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Added project context.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Continued call preparation. |
| [**Thu., July 23**](Areas/automotive-audio.md) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Created the capture-to-reproduction project structure.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Continued call preparation. |
| [**Fri., July 24**](Areas/automotive-audio.md) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Synthesized expert automotive-audio guidance.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Added the development binder.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Completed the Coffman call. |
| [**Sat., July 25**](Areas/automotive-audio.md) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Defined the Tier-1-to-OEM direction. |
| [**Sun., July 26**](Areas/critical-listening.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Initialized the technical ear-training repository.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Linked portfolio evidence.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Outlined automotive portfolio options. |

### July 27–31 — Baselines, procedures, and connected development planning

| Date | Recorded outcomes |
|---|---|
| [**Mon., July 27**](Areas/critical-listening.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Ran the listening baseline and Session 1.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Updated career direction.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Published the automotive roadmap.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Added DSP project plans. |
| [**Tue., July 28**](Areas/critical-listening.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Developed objective listening-practice procedures.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Developed the planning workflow.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Prioritized feasible automotive projects. |
| [**Wed., July 29**](Areas/critical-listening.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Refined training evidence.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Added and documented a Pure Data prototype.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Integrated portfolio evidence.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Organized the roadmap and DSP plans. |
| [**Thu., July 30**](Areas/critical-listening.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Ran Session 2 and built the warmup system.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Built the calendar and planning system.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Linked automotive work to the calendar. |
| **Fri., July 31** | [🟦 **LISTEN**](Areas/critical-listening.md) ↪ Moved the next session to Saturday.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ◇ Pure Data next step remained undecided.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Selected automotive-supporting electrical-engineering study.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Scoped weekend study.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Began the Lechner recommendation process; the signed recommendation was later received and published. |

### Month Objective

[Establish a connected portfolio and repeatable development system across technical audio disciplines.](2026-07.md#month-objective)

### Weekly Progression

- **July 1–12:** Pre-calendar development period; no consolidated record.
- [**July 13–19:** Begin the documented software and audio-analysis foundation.](2026-07.md#july-13-19)
- [**July 20–26:** Connect engineering evidence, research planning, and critical listening.](2026-07.md#july-20-26)
- [**July 27–August 2:** Establish baselines, practice procedures, and cross-repository navigation.](2026-07.md#july-27-august-2)

</details>

<details open>
<summary><strong>August 2026 — Structured Cross-Disciplinary Development</strong></summary>

## August 2026

> **Reading guide:** This main view records concise outcomes in a phone-friendly weekly agenda. Select a date or category for the fuller record.

<details>
<summary><strong>Choose an August view</strong></summary>

- **Full month:** Continue below
- [August 1–2 — Foundation and structured session](Weeks/2026-08-01-02.md)
- [August 3–9 — Benchmarks, DSP research, and trainer development](Weeks/2026-08-03-09.md)
- [August 10–16 — Trainer refinement and evaluation-system work](Weeks/2026-08-10-16.md)
- [August 17–23 — Mastery system and architecture development](Weeks/2026-08-17-23.md)
- [August 24–30 — Evidence consolidation](Weeks/2026-08-24-30.md)
- [August 31 — Monthly review](Weeks/2026-08-31.md)

</details>

### August 1–2 — Foundation and first structured session

| Date | Recorded outcomes |
|---|---|
| **Sat., Aug. 1** | [🟦 **LISTEN**](Areas/critical-listening.md) ↪ Listening setup moved to Sunday.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ [Published employer tracker](https://docs.google.com/spreadsheets/d/1RuIrnG35fnQ2G6rblekq6ydcGVg35Rl4QB69DlzJnJY/edit).<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Added consulting-employer dashboard and [initial generator/barrier workbook](https://docs.google.com/spreadsheets/d/1_Qk3HXR3SjjTd_EvE4mVO0urOPPRktQFfQgToOaHJ8E/edit).<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Set up EE path and project. |
| [**Sun., Aug. 2**](https://github.com/kevinworleymusic-afk/technical-ear-training/blob/main/logs/03-daily/2026-W31/2026-08-02_session-04.md) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Set up procedure and completed a 1,142-trial EQ session.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Researched in-car DSP options. |

### August 3–9 — Listening benchmarks, DSP research, and trainer development

| Date | Recorded outcomes |
|---|---|
| [**Mon., Aug. 3**](2026-08.md#august-3-9) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Completed 1,142+ pink-noise EQ trials and studied private Sean Olive materials.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Received local Panasonic contact.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Documented DSP learning routes.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Recorded first structured cycle. |
| [**Tue., Aug. 4**](2026-08.md#august-3-9) | [🟨 **NET**](Areas/acoustical-consulting.md) ✓ Completed Panasonic and evidence follow-ups.<br>[🟦 **LISTEN**](Areas/critical-listening.md) ↪ Moved song-EQ plan to Aug. 5. |
| [**Wed., Aug. 5**](2026-08.md#august-3-9) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Completed a 1,616-trial EQ benchmark, one-third-octave pink-noise and *Fast Car* transfer work, and downloaded seven reference tracks.<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Networking and reference outreach.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Began DSP option/tutorial research. |
| [**Thu., Aug. 6**](2026-08.md#august-3-9) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Continued DSP resource research and began tutorial/study planning. |
| [**Fri., Aug. 7**](2026-08.md#august-3-9) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Continued EQ development.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Began Listening Trainer development Friday night. |
| [**Sat., Aug. 8**](2026-08.md#august-3-9) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Major Listening Trainer development.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Expanded automotive listening use cases. |
| [**Sun., Aug. 9**](2026-08.md#august-3-9) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Trainer testing, session export, and repository work.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Jim added LinkedIn connection. |

### August 10–16 — Trainer refinement, applications, and evaluation-system work

| Date | Recorded outcomes |
|---|---|
| [**Mon., Aug. 10**](2026-08.md#august-10-16) | [🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Continued Listening Trainer development and expanded the EQ/module roadmap. |
| [**Tue., Aug. 11**](2026-08.md#august-10-16) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Jim follow-up.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ DSP/tutorial research and trainer work. |
| [**Wed., Aug. 12**](2026-08.md#august-10-16) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Jim follow-up.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Trainer, specification, and repository development. |
| [**Thu., Aug. 13**](2026-08.md#august-10-16) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Applied to additional Ford postings.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Trainer refinement and GitHub workflow. |
| [**Fri., Aug. 14**](2026-08.md#august-10-16) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Identified 3× +3 dB boosts near 100% reliably and 3× −3 dB cuts at about 90%.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ Refined trainer automobile settings.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Completed planning for adaptive difficulty and song transfer. |
| [**Sat., Aug. 15**](2026-08.md#august-10-16) | [🟦 **LISTEN**](Areas/critical-listening.md) ✓ Analyzed the Kaiser/Harman paper and developed a [subjective-listening evaluation system](../Projects/Technical-Audio-and-Research/Subjective-Listening-Evaluation-System/).<br>[🟨 **NET**](Areas/acoustical-consulting.md) ✓ Heard back from Jim Walter and received Alek Weidmann’s electronics-study book link.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ Began electronics-study and professional-development purchasing priorities; ordered [*Practical Electronics for Inventors, Fourth Edition*](https://www.amazon.com/Practical-Electronics-Inventors-Fourth-Scherz/dp/1259587541).<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Published the portfolio demo and documentation. |
| [**Sun., Aug. 16**](2026-08.md#august-10-16) | — No completed entry recorded yet. |

### August 17–23 — Mastery system and architecture development

| Date | Recorded outcomes |
|---|---|
| [**Mon., Aug. 17**](2026-08.md#august-17--mastery-operating-system-build) | [🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ ~12-hour mastery-system build and portfolio reorganization: created the Automotive Audio Mastery Registry, organized existing and planned work into Buckets 01–11, reconciled project/feature states and mappings, built dashboard/bucket-map/registry/schedule/history views, and established evidence, workflow, and bounded-development rules. |
| [**Tue., Aug. 18**](2026-08.md#august-18--vehicle-noise-research--core--focus-baseline-10) | [🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ 150 min vehicle-noise research and Bucket 03 planning; developed P-039 and six modules.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ 420 min completed **Core + Focus Baseline 1.0 — Purpose-Driven Cabin Audio Architecture**, including product vision, system/channel architecture, operating behavior, feature index, research roadmap, citations, and final editorial/branding refinement.<br>[🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ Closed the day at 570 actual minutes across two meaningful work blocks; listening was intentionally skipped and Audio Weaver was not completed. |
| [**Wed., Aug. 19**](Daily-Logs/2026-08-19.md) | [🟩 **PLAN**](Areas/portfolio-and-planning.md) ✓ ~240 min mastery-system/bucket development.<br>[🟧 **AUTO/TECH**](Areas/automotive-audio.md) ✓ 282 min electrical/electronics fundamentals plus **37+ prompt-based automotive-audio tuning/diagnostic scenarios**, with initial reasoning, review/refinement, vocabulary work, and engineering-sentence practice preserved in the linked evidence journal.<br>[🟪 **DSP/SW**](Areas/dsp-and-software.md) ✓ 60 min bounded Audio Weaver tutorial continuation.<br>✓ **582 actual minutes total.** |
| **Thu., Aug. 20** | ◇ Continue evidence-producing bucket rotation. |
| **Fri., Aug. 21** | ◇ Continue evidence-producing bucket rotation. |
| **Sat., Aug. 22** | ◇ Continue evidence-producing bucket rotation. |
| **Sun., Aug. 23** | ◇ Review the week and carry forward only defined next actions. |

### Remaining August plan

| Period | Planned progression |
|---|---|
| [**Aug. 24–30**](2026-08.md#august-24-30) | ◇ Produce or consolidate reviewable evidence. |
| [**Aug. 31**](2026-08.md#august-31) | ◇ Review the month and establish September priorities. |

**August 6–13 reconstruction note:** This period was not logged day by day at the time. Its entries were reconstructed later from available GitHub commits, saved project artifacts, confirmed outreach/application dates, and contemporaneous project or conversation records. Work was assigned to an exact day only where the evidence supported that date; otherwise it is represented as a broader period-level activity. Sparse or generalized cells in this block therefore reflect limits in date-specific documentation—not a lack of technical work or progress.

**August 1 clarification:** The completed workbook now includes automotive-audio, acoustical-consulting, and Georgia music-education dashboards in the [live Google Sheet](https://docs.google.com/spreadsheets/d/1RuIrnG35fnQ2G6rblekq6ydcGVg35Rl4QB69DlzJnJY/edit). The completed consulting dashboard is the **acoustical-consulting employer research dashboard**. The separate scheduled project is the [Generator-to-Residence Barrier Noise Analysis](../Projects/Technical-Audio-and-Research/Generator-to-Residence-Barrier-Noise-Analysis/), for which the August 1 objective was completed as an initial, AI-assisted workbook framework with a reusable calculation template and a separate Generac SG200 case-setup tab. Manufacturer-specific octave-band data, independent validation, real-site geometry, and portfolio conclusions remain future work.

### Month Objective

[Convert the portfolio foundation into coordinated, evidence-producing work across critical listening, automotive research and technical study, DSP, software, and electronics, and portfolio planning.](2026-08.md#month-objective)

### Weekly Progression

- [**August 1–2:** Establish employer research, technical-study direction, and the next listening cycle.](2026-08.md#august-1-2)
- [**August 3–9:** Begin a balanced cross-disciplinary cycle and follow the strongest evidence-producing threads.](2026-08.md#august-3-9)
- [**August 10–16:** Stabilize selected development priorities across listening, DSP/software, networking, and career search.](2026-08.md#august-10-16)
- [**August 17–23:** Build the mastery operating system, reorganize the portfolio into the bucket architecture, and begin evidence-driven system-definition work.](2026-08.md#august-17-23)
- [**August 24–30:** Produce or consolidate reviewable evidence.](2026-08.md#august-24-30)
- [**August 31:** Review the month and establish September priorities.](2026-08.md#august-31)

</details>

## Detailed Calendars

- [All detailed calendars](Calendars/)
- [Critical Listening](https://github.com/kevinworleymusic-afk/technical-ear-training/tree/main/calendar)
- [Automotive Audio & Technical Skills](Calendars/automotive-audio.md)
- [DSP, Software & Electronics](Calendars/dsp-and-software.md)
- [Portfolio & Planning](Calendars/portfolio-and-planning.md)
- [Professional Networking & Acoustical Consulting](Calendars/acoustical-consulting.md)